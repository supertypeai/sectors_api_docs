...
4. Indonesia (pop 280m, 1.48t usd): ? 

Despite being the 4th most populous country, Indonesia has the 16th largest economy and 19th largest market (23rd place is Singapore, 26th place Thailand, 28th place Malaysia).

So it's really early times, and plenty of reasons to be optimistic about the continued growth of this economy and financial market. Without further delay, let's start our journey now!
...
4. Once authorized, provide the URL of the JSON data source you want to use in the `Enter the URL of a JSON data source` field. For example, use the desired API endpoint URL. Then, click the `Add` button.  
