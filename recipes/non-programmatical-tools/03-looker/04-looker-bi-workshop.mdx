---
title: 'IDX Market Overview Dashboard'
sidebarTitle: "4. IDX Market Overview Dashboard"
description: "By: [Aurellia Christie](https://www.github.com/AurelliaChristie) & [Gerald Bryan](https://www.github.com/geraldbryan) · December 13, 2024"
---

## IHSG Market Overview
In this recipe, we will create an end-to-end IDX market overview dashboard. IDX market or usually also known as IHSG right now is one of the most promising stock exchange in the world

![idx_condition](/recipes/non-programmatical-tools/03-looker/asset/idx_position.png)

Something you didn't know about the Indonesia Stock Exchange. Based on World Federation Exchange (WFE) data per November 2024, it ranks 19nd largest, ahead of Singapore Exchange -- but only half the size of Korean S.E, Theran S.E, etc.
you can access the full data about Indonesian stock market condition [here](https://sectors.app/why-indonesia-stock-market) 

But it's experiencing an explosive growth. 
41% over 2 years. 
For perspective, here are the top 4 country by population and their GDP rank
1. India (pop 1.44b, gdp 3.9t usd): 5th largest 
2. China (pop 1.43b, gdp 18.5t usd): 2nd largest
3. US (pop 341.8m, gdp 28.8t usd): 1st largest
4. Indonesia (pop 280m, 1.48t usd): ___ can you guess where are we on the GDP rank currently? 

Despite being the 4th most populous country, Indonesia has the 16th largest economy and 22nd largest market (23rd place is Singapore, 584b market cap, 26th place Thailand, 28th place Malaysia).

So it's really early times, and plenty of reasons to be optimistic about the continued growth of this economy and financial market. Without further delay, let's start our jorney now!

## Data Fetching and Data Connector in Looker Studio

### Data Fetching using Google Sheets
In this recipe, we'll use Google Sheets to fetch data from the Sectors API. Note that this is not the only way to access data from the Sectors API; 
...
Before getting started, you’ll need two tools: the API Connector extension and a Sectors API key. For instructions on how to use the API Connector extension in Google Sheets, refer to [this tutorial in the previous recipe](../02-sheets/02-sheets#api-connector).
...
<Accordion title="Historical LQ45 Index Price">
    - **Definition:** The 30 days historical LQ45 Index Price
    - **URL:** `https://api.sectors.app/v1/index-daily/lq45/`
</Accordion>
<Accordion title="Historical IDX30 Index Price">
    - **Definition:** The 30 days historical IDX30 Index Price
    - **URL:** `https://api.sectors.app/v1/index-daily/idx30/`
</Accordion>
<Accordion title="Historical JII70 Index Price">
    - **Definition:** The 30 days historical JII70 Index Price
    - **URL:** `https://api.sectors.app/v1/index-daily/jii70/`
</Accordion>
<Accordion title="Top 10 Companies based on Market Capitalization">
    - **Definition:** The top 10 companies based on market capitalization parameter in the current year
    - **URL:** `https://api.sectors.app/v1/companies/top/?classifications=market_cap&n_stock=10`
</Accordion>
<Accordion title="Top 10 Companies based on Dividend Yield">
    - **Definition:** The top 10 companies based on dividend yield parameter in the current year
    - **URL:** `https://api.sectors.app/v1/companies/top/?classifications=market_cap&n_stock=10`
</Accordion>
<Accordion title="Top 10 Companies based on Revenue">
    - **Definition:** The top 10 companies based on revenue parameter in 2023
    - **URL:** `https://api.sectors.app/v1/companies/top/?classifications=revenue&n_stock=10&year=2023`
</Accordion>
<Accordion title="Top 10 Companies based on Earnings">
    ...
    - **URL:** `https://api.sectors.app/v1/companies/top/?classifications=earnings&n_stock=10&year=2023`
</Accordion>
<Accordion title="Top 10 Thirty Days Gainers">
    - **Definition:** The top 10 companies with the highest percentage change in price in the last 30 days
    - **URL:** `'https://api.sectors.app/v1/companies/top-changes/?classifications=top_gainers&n_stock=10&periods=30d'`
</Accordion>
<Accordion title="Top 10 Thirty Days Losers">
    - **Definition:** The top 10 companies with the lowest percentage change in price in the last 30 days
    - **URL:** `'https://api.sectors.app/v1/companies/top-changes/?classifications=top_losers&n_stock=10&periods=30d'`
</Accordion>
...
After we use the google sheets as the intermediaries to fetch the data from the API before import them into looker studio, now we will directly call the API in the looker studio using Windsor.ai connector.
For a disclaimer, Supertype is _not_ affiliated with Windsor.ai, and we are not responsible for anything that happens because of the usage of the respective extension.

- First, you need to open looker studio and create a blank report
- Then, "Add data to report" menu should be automatically pop up into the screen, and in the search box you can type `json` and in the `Partner Connectors` section you can choose the one by windsor.ai or the one in the red square.
![json_windsor](/recipes/non-programmatical-tools/03-looker/asset/windsor_connector.png)
- After choosing it, you can connect your account to windsor.ai by clicking the `authorize`. Please note, to read the terms and condition and what you allow in the authorization, once again we are not responsible for anything that happens because of the usage of the respective extension.
![windsor_authorize](/recipes/non-programmatical-tools/03-looker/asset/windsor_authorize.png)
- After the authorization, the we can put the URL link of the JSON data that we want to use in the `Enter the URL of a JSON data source`. For example, in this tutorial we will use this URL endpoint `we are not responsible for anything that happens because of the usage of the respective extension.` Then, you can click the `Add` button
![windsor_connect](/recipes/non-programmatical-tools/03-looker/asset/windsor_url.png)
- Finally, we can use the data in our report to do some visualization
![windsor_visualization](/recipes/non-programmatical-tools/03-looker/asset/windsor_visualization.png)
- Specifically, for the windsor.ai json connector, we must do another additional step to make our data is not accessible for anyone out there, since by default if we connect our data using windsor.ai it can be used by the other people in the community. To disable that settings, we can go to `Resources` menu -> `Manage added data sources` -> `Edit` in the specific dataset that use windsor.ai data.
![windsor_data_source](/recipes/non-programmatical-tools/03-looker/asset/windsor_data_source.png)
- Then we can click on the `Community visualization access`, and set it to `off` and click `save`
...
To change the data source from embedded into reusable you just need to open the report which you use the desired data. Then go to `Resource` menu -> `Manage added data sources`, and click the `Make Reusable` in the desired dataset. 
![reuseable_data](/recipes/non-programmatical-tools/03-looker/asset/reuseable_data.png)

After you click that, your dataset could be imported directly into another dashboard, without configuring connection for every report. You just need to go to `+Add Data` menu -> switch to `My data sources` and select the data you want to used.
![my_data_source](/recipes/non-programmatical-tools/03-looker/asset/my_data_source.png)

## The Fundamental of Data Visualization

Before we dive in into the practical work on Looker Studio, let's start with some theoretical knowledge first about data visualization and data storytelling
