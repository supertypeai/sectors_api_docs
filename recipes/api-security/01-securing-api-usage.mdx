...
```
##### **Insecure HTTP Request (DO NOT USE)**
```python
url = "http://api.sectors.app/v1/company/report/<company-name>"  # Incorrect, unencrypted request
response = requests.get(url)
```
### B. SSL/TLS Certificate Verification
#### Why Verify SSL Certificates?
- SSL/TLS certificates authenticate the API server and encrypt data in transit.
- If an attacker intercepts traffic and serves a fake API endpoint (e.g., via DNS spoofing), your client must detect it.
- Without SSL verification, an attacker could intercept requests and manipulate data.

#### How to Ensure SSL Certificate Verification in Python
By default, Python’s requests module automatically verifies SSL certificates, but you should always check explicitly:

##### **Enforcing SSL Verification (Secure)**
```python
...
# Expected output: SSL certificate verified!
```
##### **Disabling SSL Verification (NOT RECOMMENDED)**
```python
response = requests.get(url, verify=False)  # Disables SSL verification
```
If you disable SSL verification, attackers could intercept and manipulate API responses. Never disable SSL verification unless debugging a local API with self-signed certificates.


### C. Additional Encryption Practices
#### Certificate Pinning
Certificate pinning ensures that your application only trusts specific SSL certificates from the API provider, reducing the risk of rogue certificate authorities (CA) issuing fake certificates.

##### How to Implement Certificate Pinning in Python
```python
import requests

# Define the expected certificate fingerprint (SHA-256 of public key)
VALID_CERTIFICATE_HASH = "d2a57f5b4e6ff865d849..."
...
def check_certificate(url):
    ...
    # Verify the certificate matches expected hash
    if cert_hash == VALID_CERTIFICATE_HASH:
        print("Certificate is valid and trusted!")
    else:
        print("WARNING: Untrusted certificate!")

check_certificate("https://api.sectors.app")

# Expected output: Certificate is valid and trusted!
```
#### Using TLS 1.2+
- Ensure you are using TLS 1.2 or TLS 1.3 for enhanced security.
- You can check the TLS version used in your Python request:
```python
import ssl
print(ssl.OPENSSL_VERSION)

# Expected Output (Example for TLS 1.2 or higher): OpenSSL 1.1.1  11 Sep 2018
```
