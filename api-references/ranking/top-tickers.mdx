...
<ParamField query="filters" type="string" default="">
  ...
  <AccordionGroup>
    ...
    <Accordion title="Targets">
      Use the same targets as classifications (except for <code>market_cap</code> which is replaced by <code>min_mcap_billion</code>).
    </Accordion>
  </AccordionGroup>
</ParamField>

<ParamField query="logic" type="string" default="and">
  Logical operator to combine multiple filters. Accepts <code>`and`</code> or{" "}
