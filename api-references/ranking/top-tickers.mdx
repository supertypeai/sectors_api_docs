...
<ParamField query="sub_sector" type="string" default="all">
  Kebab-case subsector filter (e.g. `banks`, `consumer-services`). Mutually
  exclusive with `sub_industry`.
</ParamField>

<ParamField query="sub_industry" type="string" default="all">
  Kebab-case sub-industry filter (e.g. `automotive`, `pharmaceuticals`).
  Mutually exclusive with `sub_sector`.
</ParamField>
