...
api: "GET /v2/company/get-segments/{ticker}/"
...
<ParamField path="ticker" type="string" required>
  ...
  <AccordionGroup>
    <Accordion title="Get the available ticker">
      Get the available `ticker` from the [Companies with Revenue
      Segments](../helper-list/companies-segments-list) endpoints.
    </Accordion>
...
<ParamField query="financial_year" type="integer">
  ...
  <AccordionGroup>
    <Accordion title="Get the available financial_year">
      Get the available `financial_year` from the [Companies with Revenue
      Segments](../helper-list/companies-segments-list) endpoint.
    </Accordion>
