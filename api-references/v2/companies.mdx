...
<ParamField query="q" type="string">
  ...
  <Accordion title="Query Capabilities & Logic">
    - **Smart FY Handling**: To ensure data reliability during transition periods (Jan–Apr), the system intelligently prioritizes the most complete data sets. If a query is made for the current year before fresh filings are fully available, the system automatically defaults to the previous year’s full fiscal data to provide a robust baseline for your analysis.
    - Note for Enhanced Accuracy: For the most precise results, we recommend filtering by specific sectors or industries using our standardized slugs. You can retrieve the complete list of official slugs from [Subsectors](https://docs.sectors.app/api-references/helper-list/subsectors), [Subindustries](https://docs.sectors.app/api-references/helper-list/subindustries), or [Industries](https://docs.sectors.app/api-references/helper-list/industries).
  </Accordion>
</ParamField>
...
<ParamField query="include_query_values" type="boolean" default="false">
  If set to `true`, the response will include a `query_values` object showing the
  interpreted values used for the query (e.g., extracted year, country).
</ParamField>

### Response

<ResponseField name="query_values" type="object">
  <Expandable title="Query Values properties">
    <ResponseField name="year" type="integer">
      The year extracted from the query, if any.
    </ResponseField>
    <ResponseField name="country" type="string">
      The country name extracted from the query, if any.
    </ResponseField>
  </Expandable>
</ResponseField>
